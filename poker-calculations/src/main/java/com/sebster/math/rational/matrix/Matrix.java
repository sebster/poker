...
public class Matrix implements Cloneable, java.io.Serializable {
	...
	public Matrix(int m, int n, Rational s) {
		this.m = m;
		this.n = n;
		a = new Rational[m][n];
		for (int i = 0; i < m; i++) {
	...
	public Matrix(Rational[][] a, int m, int n) {
		this.a = a;
		this.m = m;
		this.n = n;
	}
	...
	public Matrix(Rational vals[], int m) {
		this.m = m;
	...
	public static Matrix constructWithCopy(Rational[][] a) {
		int m = a.length;
		int n = a[0].length;
		Matrix x = new Matrix(m, n);
		Rational[][] c = x.getArray();
		for (int i = 0; i < m; i++) {
			if (a[i].length != n) {
				throw new IllegalArgumentException("All rows must have the same length.");
			}
	...
	public Rational[][] getArrayCopy() {
		Rational[][] c = new Rational[m][n];
		for (int i = 0; i < m; i++) {
			for (int j = 0; j < n; j++) {
				c[i][j] = a[i][j];
			}
	...
	public Rational[] getColumnPackedCopy() {
		Rational[] vals = new Rational[m * n];
		for (int i = 0; i < m; i++) {
			for (int j = 0; j < n; j++) {
				vals[i + j * m] = a[i][j];
			}
	...
	public Rational[] getRowPackedCopy() {
		Rational[] vals = new Rational[m * n];
		for (int i = 0; i < m; i++) {
			for (int j = 0; j < n; j++) {
				vals[i * n + j] = a[i][j];
	...
	public void set(int i, int j, Rational s) {
		a[i][j] = s;
	}
	...
	public Matrix times(Rational s) {
		Matrix X = new Matrix(m, n);
		Rational[][] C = X.getArray();
		for (int i = 0; i < m; i++) {
			for (int j = 0; j < n; j++) {
	...
	public Matrix timesEquals(Rational s) {
		for (int i = 0; i < m; i++) {
			for (int j = 0; j < n; j++) {
...
//	}
	...
	public static Matrix identity(int m, int n) {
		...
		for (int i = 0; i < m; i++) {
			for (int j = 0; j < n; j++) {
				X[i][j] = (i == j ? Rational.ONE : Rational.ZERO);
			}
		}
		return A;
	}
