...
public class GapSizeCalculator {
	...
	public static void main(final String[] args) throws Exception {
		...
		final ChipModel chipModel = IndependentChipModel.getInstance();
		...
		for (int left = seats; left > 1; left--) {
			...
			stacksAfter[0] *= 2;
			double[] equities = EquityCalculator.calculateEquities(stacksBefore, prizes, chipModel);
			double[] equitiesAfter = EquityCalculator.calculateEquities(stacksAfter, prizes, chipModel);
			double equityAfterL = left > prizes.length ? 0D : prizes[left - 1];
